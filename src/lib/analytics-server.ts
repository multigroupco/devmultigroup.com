...
export function posthogConfig(env: Env, settings?: Settings): PhConfig | null {
  ...
  if (!key) return null;
  const host = (settings?.posthog_host || env.POSTHOG_HOST || "https://us.i.posthog.com").replace(/\/$/, "");
  return { key, host };
}
...
export function captureServer(
  ...
  if (!opts.request) return Promise.resolve();
  try {
    if (!isApexHost(new URL(opts.request.url).host)) return Promise.resolve();
  } catch {
    return Promise.resolve();
  }

  const cookieId = opts.request ? distinctIdFromRequest(opts.request, cfg.key) : null;
  const distinctId = opts.distinctId || cookieId || `srv_${crypto.randomUUID?.() ?? Date.now()}`;
  ...
  if (opts.request) {
    ...
    // Deliberately do NOT forward the visitor's cf-connecting-ip. The opt-out /
    // DNT consent decision is client-side only (localStorage) and never reaches
    // the Worker, so omitting the raw IP keeps un-consented visitors' addresses
    // out of PostHog. "0.0.0.0" tells PostHog to skip IP storage + GeoIP.
